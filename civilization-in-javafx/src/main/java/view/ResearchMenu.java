...
public class ResearchMenu {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            for (Technology technology : technologies) {
                ...
                if (technology.getLevel() == i) {
                    ...
                    techBackground.addEventHandler(MouseEvent.MOUSE_CLICKED, e -> {
                        if (e.isControlDown()) {
                            if (techController.isTechAvailable(user, technology))
                            user.getTechnologiesName().add(technology.getName());
                        }
                    });
    ...
    private ImageView findTechBackground(User user, Technology technology) {
        if (user.getTechnologiesName().contains(technology.getName()))
            return new ImageView(images.completeTechBackground);
        else if (user.getCurrentTechnology() != null && user.getCurrentTechnology().getName().equals(technology.getName()))
